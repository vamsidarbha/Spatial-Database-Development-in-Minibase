...
/** get the length of a tuple, call this method if you did 
  ...
  */
  ...
  {           
   ...
   public Shape_Geometry getShape_Geometry(int fldNo)
   throws IOException, FieldNumberOutOfBoundException, ClassNotFoundException 
   {
	   Shape_Geometry val;
	   //byte[] newData=getByteDataInObjectStream(tuple1);
	   if ( (fldNo > 0) && (fldNo <= fldCnt))      
       {
        val = Convert.getShape_Geometry(fldOffset[fldNo -1], data);
        return val;
       }
      else 
       throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
  ...
  public byte[] getByteDataInObjectStream(Tuple tuple1) throws IOException {
	// TODO Auto-generated method stub
	  ByteArrayOutputStream bos = new ByteArrayOutputStream();
	  ObjectOutput out = null;
	  out = new ObjectOutputStream(bos);   
	  out.writeObject(tuple1);
...
/**
  ...
  public Tuple setShapeFld(int fldNo,Shape_Geometry val) throws IOException, FieldNumberOutOfBoundException
  {
	  { 
		    if ( (fldNo > 0) && (fldNo <= fldCnt))
		     {
			Convert.setShapeGeoValue (val, fldOffset[fldNo -1], data);
			return this;
		     }
		    else 
		     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND"); 
